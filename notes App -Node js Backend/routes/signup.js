...
router.post('/', SIGNUP_ExpressValidation, async (req, res) => {
    ...
    try {
        ...
        console.log(`Find Email db : ${FindEmail}`)

        if (!FindEmail) {
            const SaveData = await SignUp.save(); // save data in database

            // // JWT
            const Token = jwt.sign({ id: SaveData._id , Email: SaveData.Email}, process.env.TOKEN_SCRETE_KEY);
            ...
            if (Compare_Password) {
                ...
                console.log(`signup - Already created user  : ${Token}`)
                ...
                console.log(`Email & Password invalid ( Password WRONG Typed ${req.Password} )  req-body-Email : ${Email}`)
